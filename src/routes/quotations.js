...
router.post('/deleteReceived', async (req, res, next) => {
    ...
    for (const receivedQuotation of user.receivedQuotations) {
        if (receivedQuotation._id.toString() === receivedQuotationId) {
            ...
            if (currentReceivedQuotation.finishedLocked === true) {
                res.send({
                    code: 400,
                    msg: '报价单已完成报价，不可拒绝报价'
                });
...
router.post('/finished', async (req, res, next) => {
    ...
    // check if exist
    const checkReceivedQuotation = await ReceivedQuotation.findOne({
        publisher: targetUser._id,
        receiver: currentUserId,
        quotation: quotation
    });

    if (!checkReceivedQuotation) {
        res.send({
            code: 400,
            msg: '报价单不存在'
        });
    ...
    if (checkReceivedQuotation.finishedLocked === true) {
        res.send({
            code: 400,
            msg: '报价单已完成报价，不可重复报价'
        });
        ...
        checkReceivedQuotation.finishedLocked = true;
        await checkReceivedQuotation.save();
    }
